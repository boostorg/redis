...
Boost.Redis supports Redis Sentinel deployments. Sentinel handling is built-in:
...
automatically connect to Sentinels, resolve the master's address, and connect to the master.
...
Once set, the connection object can be used normally. See our Sentinel examples for full program listings:

* {site-url}/example/cpp20_sentinel.cpp[cpp20_sentinel.cpp] (Asio)
* {site-url}/example/corosio_sentinel.cpp[corosio_sentinel.cpp] (Corosio)
...
This section details how `connection::async_run` and `co_connection::run` interact with Sentinel.
