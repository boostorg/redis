...
For this reason, it is usually a good idea to set a timeout to request execution.
The specifics depend on whether you are using Asio or Corosio, but the
...
See our {site-url}/example/corosio_timeouts.cpp[example on timeouts]
for a full code listing.
...
We mentioned that executing a request waits until the server is up
