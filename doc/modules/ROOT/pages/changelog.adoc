...
* (Pull request 428) Fixed a case of undefined behavior within `connection`
...
* (Pull request 415) Optimized buffer rotations to reduce CPU consumption
  when many small messages arrive in a single network packet.
* (Pull request 424) Added initializers to all members in the `config` struct.
  This allows using designated initializers to create `config` objects.
* (Pull request 409) Removed the standalone workflow from the CMake.
