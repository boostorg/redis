...
* Call xref:reference:boost/redis/basic_connection.adoc[`connection::set_receive_response`] /
  xref:reference:boost/redis/co_connection/set_receive_response.adoc[`co_connection::set_receive_response`]
...
* Loop until the connection is cancelled.
  In Asio, gate the loop on xref:reference:boost/redis/basic_connection.adoc[`connection::will_reconnect`].
  In Corosio, run until xref:reference:boost/redis/co_connection/receive.adoc[`co_connection::receive`]
  reports an error (e.g. cancellation when the surrounding `when_any` tears down the run loop).
* Call xref:reference:boost/redis/basic_connection.adoc[`connection::async_receive2`] /
  xref:reference:boost/redis/co_connection/receive.adoc[`co_connection::receive`]
...
The pending count is reset each time `async_receive2` (Asio) /
`receive` (Corosio)
completes. You must still free
...
from within the receiver loop. The response to your request may be behind enough pushes might to trigger the flow control mechanism, causing a deadlock.


== Subscribe confirmations

Every time you subscribe to a channel, Redis sends a push as a confirmation. These confirmations
are stored in your receive response and cause `async_receive2` (Asio) /
`receive` (Corosio)
to complete like any
other push. `push_parser` skips these messages, since they don't contain application-level information.

As a result, *the receive call will complete while the `push_parser` range is empty*
...
// DON'T DO THIS: more pushes may be read while the WebSocket send runs,
// and they are discarded when you clear(), causing a race.
co_await websocket.send(push_parser(resp.value()));
resp.value().clear();
----
--
========
